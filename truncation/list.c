 ...
 * 把pos右边的往右移动一格，把element放在pos的位置上
 */
 ...
 * 把pos上的element移走，把pos右边的整体往左移动一格
 */
...
void list_map2 (void (*f) (element_t*, element_t, element_t), struct list* out_list, struct list* in_list0, struct list* in_list1) {
  ...
  // printf("works in figuring out size\n");

  for (int i = 0; i < n; i++) {
    if (out_list->len==out_list->size) {
      expand_list(out_list);
      //printf("works in resizing\n");
    }
    // printf("%d\n", i);
    element_t rv = 0;
...
void list_filter (int (*f) (element_t), struct list* out_list, struct list* in_list) {
  ...
  int n = list_len(in_list);
  // if (out_list == NULL) {
  //   out_list = malloc(sizeof(struct list));
  //   out_list->data  = malloc(sizeof (element_t) * n);
  //   out_list->len   = 0;
  //   out_list->size  = n;
  // }
  ...
  for (int i = 0; i < n; i++) {
    ...
    //   expand_list(out_list);
    // }
    int true_false = f(in_list->data[i]);
    if (true_false) {
